...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const item of notifications) {
      ...
      const usersecond: any = await getUserById(
        item.sapati.created_for as string
      );
      ...
      const request =
        item.status == "REQUEST"
          ? `Request from ${item.sapati.created_user_name} `
          : item.sapati.sapati_satatus == "APPROVED" &&
            item.status !== "SETTLED" &&
            item.status !== "DUE"
          ? `Sapati Approved from ${item.sapati.fullName} `
          : item.sapati.sapati_satatus == "DECLINED"
          ? `Sapati Rejected from ${item.sapati.fullName} `
          : item.sapati.sapati_satatus == "CHANGE"
          ? `Sapati Change Request from ${item.sapati.fullName} `
          : item.status == "SETTLED"
          ? `Settled from ${item.sapati.settled_by_name} `
          : item.status == "DUE"
          ? `Due date has been crossed`
          : null;
      const requestDescription =
        item.status == "REQUEST"
          ...
          ? `${item.sapati.fullName} declined your request of amount <b>${item.sapati.amount}</b> `
          : item.sapati.sapati_satatus == "CHANGE" && item.status == "CHANGE"
          ? `${item.sapati.fullName} has requested to change request of amount <b>${item.sapati.amount}</b> `
          : item.status == "SETTLED"
          ? ` ${item.sapati.settled_by_name} have successfully settled sapati`
          : item.status == "DUE"
          ? `${item.sapati.fullName} Sapati of amount has to be paid back <b>${item.sapati.amount}</b> `
          : null;
      const status =
        item.status !== "SETTLED" && item.status !== "DUE"
          ? item.sapati.sapati_satatus
          : item.status;
      ...
      const userId =
        item.status === "DUE" && item.due_type === "GET"
          ? item.sapati.created_by
          : item.status === "REQUEST"
          ? item.sapati.created_by
