...
export async function PATCH(req: NextRequest, params: any) {
  ...
  try {
    ...
    if (
      sapati.borrowings?.user_id == existingToken.user_id ||
      sapati.lendings?.user_id == existingToken.user_id
    ) {
      if (sapati.sapati_satatus == "PENDING") {
        return NextResponse.json(
          { message: "You cannot settle pending sapati" },
          { status: 200 }
        );
      }
      ...
      if (settlement.created_by == settlement.settled_by) {
        const notificationUser = await getUserByPhone(
          parseInt(settlement.phone)
        );
      ...
      return NextResponse.json(
        { message: "Successfully setteled sapati", sapati },
        { status: 200 }
      );
    }
